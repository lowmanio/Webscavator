...
(function($) {
	...
	function createBar(e, opts) {
		var val = e.text();
		e.css({marginLeft: e.prev().is("dt") ? "10px" : "0px", width: Math.floor(val/opts.interval)+"px"});
	...
	function setHoverColors(bars, opts) {
		...
		bars.each(function(i) {
			...
			bar.hover(function() {
				$(this).css({backgroundColor: hc, cursor:'pointer', cursor:'hand'});
			}, function() {
				$(this).css({backgroundColor: c, cursor:'default'});
			});
		});
	}
